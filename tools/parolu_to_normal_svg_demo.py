...
PRICE_BLOCK = fitz.Rect(27.27, 343.53, 392.25, 517.07)
TOP_PRICE_RECT = fitz.Rect(27.27, 343.53, 392.25, 376.81)
ROW_RECTS = [
    (
...
BOTTOM_PRICE_RECT = fitz.Rect(27.27, 496.53, 392.25, 517.07)
...
def render_svg_pdf(
    ...
    if price_data is not None:
        ...
        def draw_price(
            ...
            number_baseline = center_y - (number_size * 0.32)

            pdf_canvas.setFont(number_font, number_size)
            pdf_canvas.drawString(start_x, number_baseline, text)
        ...
        for row_rects, values in zip(ROW_RECTS, price_data["rows"]):
            draw_price(row_rects[0], values[0], "SofiaSansSemiBold", 15, small_icon_size[0], 1.2)
            draw_tl_icon(row_rects[0], values[0], "SofiaSansSemiBold", 15, SMALL_TL_SVG, small_icon_size[0], small_icon_size[1], 1.2, ROW_TL_OFFSET_Y)
            draw_price(row_rects[1], values[1], "SofiaSansSemiBold", 15, small_icon_size[0], 1.2)
            draw_tl_icon(row_rects[1], values[1], "SofiaSansSemiBold", 15, SMALL_TL_SVG, small_icon_size[0], small_icon_size[1], 1.2, ROW_TL_OFFSET_Y)
            draw_price(row_rects[2], values[2], "SofiaSansSemiBold", 15, small_icon_size[0], 1.2)
            draw_tl_icon(row_rects[2], values[2], "SofiaSansSemiBold", 15, SMALL_TL_SVG, small_icon_size[0], small_icon_size[1], 1.2, ROW_TL_OFFSET_Y)
        draw_price(
            BOTTOM_PRICE_RECT,
            str(price_data["main_bottom"]),
            "SofiaSansSemiBold",
            15,
            small_icon_size[0],
            1.2,
...
def collect_price_spans(page: fitz.Page) -> dict[str, object]:
    ...
    return {
        "main_top": str(main_top["text"]),
        "main_bottom": str(main_bottom["text"]),
        "rows": rows,
    }
    ...
    for x0, y0, x1, y1, text, *_ in page.get_text("words"):
        ...
        if cleaned.count(".") == 2 and any(ch.isdigit() for ch in cleaned):
            footer_values.append(
                {"text": cleaned, "bbox": (x0, y0, x1, y1), "size": 7.9}
            )
            continue
        if cleaned == "Türkiye":
            footer_values.append(
...
def main() -> int:
    ...
    svg_path = Path("tmp/pdfs/normal-template.svg")
    output_pdf = Path("output/pdf/ornekk-page2-parolu-to-normal-demo-fullpage-v8.pdf")
    preview_png = Path("output/pdf/ornekk-page2-parolu-to-normal-demo-fullpage-v8-page2.png")

    source_pdf = fitz.open(stream=input_pdf.read_bytes(), filetype="pdf")
    ensure_reportlab_fonts()
    price_data = collect_price_spans(source_pdf[1])
    footer_data = collect_footer_spans(source_pdf[1])
    ...
    render_preview(output_pdf, preview_png, page_number=1)
    print(output_pdf)
    return 0
